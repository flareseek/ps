...
## 리뷰
처음 시도했던 방법은 입력받은 배열들을 queue에 담고, 수를 뒤집어야 할 때 queue에 있는 값들을 stack으로 옮긴뒤 다시 queue로 이동시키도록 구현했다. <br>
채점 결과는 시간초과가 떴다. <br>
알고리즘 분류를 보니 덱을 이용해서 풀어야 하는 문제였다. <br>
생각해보니 덱을 이용하면 다시 정렬할 필요가 없었다. <br>
처음 수를 받을때는 덱의 앞쪽을 첫번째로 두면 되고, 만약 R을 수행해야되면 덱의 뒷쪽을 앞으로 두고 풀면 됐었다. <br>
이렇게 구현하니 통과 할 수 있었다.<br>
자료구조를 어떻게 사용하는지 잘 파악해야 하는 문제였다고 생각한다.
