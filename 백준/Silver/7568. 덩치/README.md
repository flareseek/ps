...
### 출력 
 ...
 ## 리뷰
문제 이해를 잘못해서 2시간동안 풀고 있었다.<br>
n번째 등수에 있는 값들의 기준이 전부 같아야 된다고 생각했다. 그러니까 해당 등수의 몸무게와 키의 범위 값이 전부 같아야 하는줄 알았다.<br>
만약 1, 2, 3등이 있다면 2등들의 값은 1등에 있는 값에 몸무게와 키의 최소값과 3등에 있는 값에 몸무게와 키의 최대값 사이에 있어야 된다고 이해했다.<br> 
그러나 등수를 정하는 방식은 해당 값이 몇명보다 큰지 점수를 매겨, 그 값을 기준으로 등수를 정하는것이였다.<br>
<br>
사실 2시간을 붙잡고 있을 문제는 아니였다고 생각하고 문제 설명을 대충 읽어서 생긴 문제라고 본다.<br><br>

문제들을 풀때, 특히 구현문제를 풀때에는 잘 풀리지 않을 경우 문제를 잘못 이해한 경우가 많았다.<br>
앞으로 처음에 문제를 꼼꼼히 읽어서 놓친게 없는지, 문제에서 요구하는 사항이 정확히 무엇인지 다시 생각해보고<br>
문제가 잘 풀리지 않으면 내가 문제를 제대로 이해한게 맞는지 다시 확인하는 습관을 가져야겠다.
